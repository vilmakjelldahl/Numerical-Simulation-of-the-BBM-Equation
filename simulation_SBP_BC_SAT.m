...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%global variables
global a n c D1x It A R u0 D1t Ix HIx e_l e_r d1_r d2_r d2_l mt mx xvec split_approx
...
if n == 1
    % 1
    sat1 = -1/2*HIx*(e_l + 2*d2_l')  * (e_l' );
    sat2 =  1/2*HIx*(e_r +  2*d2_r')   * (e_r');
    sat3 = -1/2*HIx*d1_r' * (d1_r);
elseif n ==2
    %1
    sat1 = -1/2*HIx*(e_l + 2*d2_l')  * (e_l' );
    sat2 =  1/2*HIx*(e_r +  2*d2_r')   * (e_r');
...
for i_block = 1:n_blocks
    ...
    while abs(normest(du)) > tol
        %SAT, non-linear terms
        %%%%
        SAT_term2 = [];
        for ii = 1:mt
            vl = e_l'*u0(mx*(ii-1) + 1:mx*ii);
            vr = e_r'*u0(mx*(ii-1) + 1:mx*ii);
            
            if n == 1
                %1
                SAT1 = -1/2*HIx*e_l   * (   2*a/3*vl^2  );
                SAT2 =  1/2*HIx*e_r   * (   2*a/3*vr^2 );
            elseif n == 2
                %1
                SAT1 = -1/2*HIx*e_l   * (   2*a/3*vl^2  );
                SAT2 =  1/2*HIx*e_r   * (   2*a/3*vr^2 );
            end  
            SAT_term2 = [SAT_term2 ; (SAT1+SAT2)];
        end
        %%%%

        %construct b
        b = (Dx_bar-Dt_bar)*u - R*u0 + F(u) + SAT_term2 + SAT_term1*u;
